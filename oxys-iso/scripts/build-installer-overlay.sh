...
# --- Portage-owned oxys CLI -------------------------------------------------
# Build the static CLI into app-admin/oxys's FILESDIR, generate its Manifest,
# and synchronize the canonical first-party overlay into the ISO. Catalyst
# emerges this ebuild during stage1, so /usr/bin/oxys has a real VDB owner and
# can later bootstrap itself through a binary package update.
OXYS_PACKAGE_STAGE="${MONOREPO_ROOT}/oxys-build/scripts/stage-oxys-package.sh"
"${OXYS_PACKAGE_STAGE}" \
	--cpu-baseline "${OXYS_CPU_BASELINE:-x86-64-v3}"

# Never retain the pre-package-manager delivery path. /usr/local/bin precedes
# /usr/bin on normal PATHs and would silently shadow a newly updated package.
rm -f "${REPO_DIR}/overlay/usr/local/bin/oxys"
echo ">> staged Portage-owned oxys CLI package (cpu=${OXYS_CPU_BASELINE:-x86-64-v3})"
