...
use crate::manifest::{Disk, DiskLayout, SystemManifest};
...
pub(super) fn write_timezone(
    ...
    write_file(
        &target_mount.join("etc/timezone"),
        &format!("{timezone}\n"),
    )?;
...
mod tests {
    ...
    fn seed_writes_source_and_manifest() {
        ...
        assert!(toml.contains("oxys_checksum_sha256"), "applied state must be checksummed");
    }
