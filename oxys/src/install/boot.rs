...
pub(super) fn write_systemd_boot(
    ...
    let options = boot_options(manifest)?;
...
pub(super) fn write_grub_config(
    ...
    if manifest.disk.layout == DiskLayout::Zfs {
        return write_zfs_grub_config(manifest, target_mount, &grub_dir);
    }
    ...
    if esp_root.join("EFI/oxys/initramfs.img").exists() {
...
fn boot_options(manifest: &SystemManifest) -> Result<String, SystemInstallError> {
    ...
    options.extend(manifest.kernel.cmdline.iter().cloned());
    Ok(options.join(" "))
}
